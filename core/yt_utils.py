...
from .gemini_client import get_gemini_client, generate_with_retry
...
# googleapiclient's execute() takes no per-call timeout kwarg; set it once on
# the transport instead of mutating socket.setdefaulttimeout() per call, which
# would be a global (process-wide) and thread-unsafe knob once the app runs
# under gthread with multiple concurrent requests.
_YT_METADATA_TIMEOUT_S = 10
youtube = build('youtube', 'v3', developerKey=_yt_api_key,
                 http=httplib2.Http(timeout=_YT_METADATA_TIMEOUT_S))
